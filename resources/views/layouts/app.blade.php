...
<body class="bg-yellow-50 text-gray-800 antialiased">
    ...
    {{-- Ambil status AI (Opsional, pakai default true jika null) --}}
    @php
    ...
    <div class="flex flex-col h-full w-full overflow-hidden relative">
        ...
        <div class="flex-1 flex overflow-hidden relative">
            ...
            {{-- SIDEBAR RIWAYAT --}}
            <aside id="chat-sidebar" class="fixed inset-y-0 left-0 w-72 bg-white/95 backdrop-blur-md border-r border-yellow-200 z-40 transform -translate-x-full transition-transform duration-300 md:relative md:translate-x-0 md:flex flex-col h-full shadow-2xl md:shadow-none">
                ...
                <div class="flex-1 overflow-y-auto px-3 custom-scrollbar space-y-2 pb-4">
                    ...
                    @forelse($histories as $history)
                        <div class="group flex items-center justify-between px-3 py-3 rounded-xl transition-all border border-transparent {{ (isset($currentChat) && $currentChat->id == $history->id) ? 'bg-blue-50 border-blue-200 shadow-sm' : 'hover:bg-yellow-50 hover:shadow-sm' }}">
                            ...
                            {{-- TOMBOL HAPUS --}}
                            <form action="{{ route('chat.delete', $history->id) }}" method="POST" onsubmit="return confirm('Hapus chat ini?');">
                                @csrf @method('DELETE')
                                <button type="submit" class="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-600 p-1">🗑️</button>
                            </form>
                        </div>
                    @empty
            ...
            <main class="flex-1 flex flex-col h-full relative z-10 bg-white/30 backdrop-blur-sm w-full">
                <div id="chat-box" class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 scroll-smooth custom-scrollbar w-full">
                    @if(!isset($currentChat) || $messages->count() == 0)
                        <div class="flex flex-col items-center justify-center h-full text-center space-y-4 px-4 animate-slide-up">
                            ...
                            <div>
                                ...
                                <p class="text-gray-500 font-bold mt-2 bg-white/80 inline-block px-4 py-2 rounded-full border border-white shadow-sm">Siap bertualang? Ketik sesuatu di bawah! 🚀</p>
                            </div>
                        </div>
                    @else
                ...
                <div class="flex-shrink-0 p-3 md:p-4 bg-white/90 backdrop-blur-lg border-t border-yellow-200 z-20 w-full">
                    <form id="chat-form" action="{{ route('chat.send') }}" method="POST" class="relative flex items-center gap-2 max-w-4xl mx-auto w-full">
                        @csrf
                        @if(isset($currentChat)) <input type="hidden" name="chat_id" value="{{ $currentChat->id }}"> @endif
                        <div class="relative flex-1 group">
                            <input type="text" name="message" id="user-input" maxlength="250" class="w-full border-4 border-blue-100 rounded-full py-3 md:py-4 pl-5 pr-16 focus:border-pink-400 focus:ring-4 focus:ring-pink-100 transition-all font-bold text-gray-700 placeholder-gray-400 bg-white shadow-inner text-sm md:text-base disabled:bg-gray-100 disabled:cursor-not-allowed" placeholder="{{ $aiStatus ? 'Ketik pertanyaanmu di sini...' : '⛔ AI sedang Istirahat' }}" autocomplete="off" required {{ $aiStatus ? '' : 'disabled' }}>
                        </div>
                        <button type="submit" id="send-btn" {{ $aiStatus ? '' : 'disabled' }} class="flex-shrink-0 w-12 h-12 md:w-14 md:h-14 bg-gradient-to-r from-pink-500 to-orange-500 rounded-full text-white flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 border-4 border-white">➤</button>
                    </form>
                </div>
            </main>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if(chatForm) {
                chatForm.addEventListener('submit', function(e) {
                    ...
                    input.value = ''; sendBtn.disabled = true; scrollToBottom();
                    ...
                    .then(data => {
                        ...
                        // LOGIKA PENTING: Update URL tanpa refresh agar riwayat tersimpan di sesi ini
                    ...
                    .finally(() => { sendBtn.disabled = false; input.focus(); scrollToBottom(); });
                });
            }
            ...
            function addBubble(text, role) {
                ...
                div.innerHTML = `<div class="w-8 h-8 rounded-full ${role === 'user' ? 'bg-green-500' : 'bg-pink-500'} border-2 border-white flex-shrink-0 flex items-center justify-center text-white text-xs shadow-md">${role === 'user' ? '🧑‍🚀' : '🤖'}</div><div class="${bg} p-3 px-4 rounded-2xl shadow-sm max-w-[85%] text-sm md:text-base font-medium leading-relaxed">${text}</div>`;
                scrollAnchor ? chatBox.insertBefore(div, scrollAnchor) : chatBox.appendChild(div);
                scrollToBottom();
            }
